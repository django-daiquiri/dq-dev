...
def init(args: argparse.Namespace):
    ...
    clean_temp_files(conf["basedir"], conf["conf"].get("enable_containers", None))

    if conf["conf"].get("custom_scripts", None) is not None:
        copy_custom_scripts(
    ...
    if conf["conf"].get("exposed_ports", None) is not None:
...
def clean_temp_files(basedir: Path, container_names: list[str] | None):
...
def parse_ports(conf: dict) -> dict:
    portmap = {}
    for service_name in conf["conf"].get("enable_containers", []):
        r = None
        try:
